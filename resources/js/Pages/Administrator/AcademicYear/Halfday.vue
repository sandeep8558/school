...
export default {
    ...
    methods: {
        ...
        submit(){
            ...
            this.form.post('/academic_year/save_halfday', {
                onFinish: data => {
        ...
        async dblClick(e){
            ...
            const options = {
                ...
                body: JSON.stringify({ on_date: e.on_date, grade_id: e.grade_id, academic_year_id: e.academic_year_id, note: 'Halfday', _token: this.$page.props.csrf_token })
            };

            await fetch('/academic_year/add_remove_halfday', options)
...
<template>
    ...
    <Administrator>
        ...
        <div class="box">
            ...
            <div class="w-full">
                ...
                <div class="flex items-center">
                    <h2 class="text-2xl font-bold w-full">Halfday for {{ academic_year ? academic_year.name : '' }}</h2>
                    <div class="w-80">
                        <button class="btn float-right" :class="isForm ? 'btn-red' : 'btn-orange'" @click="isForm = !isForm;">
                            <span>{{ isForm ? 'Close Form' : 'Set Halfday' }}</span>
                        </button>
                    </div>
                </div>
        ...
        <Calendar @onOptionsChange="optionsChanged($event)" @onDblClick="dblClick($event)" :halfdays="halfdays"></Calendar>
